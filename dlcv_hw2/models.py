...
class VGG(nn.Module):
    ...
    def __init__(self, features, image_size=448, grid_num=7):
        ...
        self.yolo = nn.Sequential(
            nn.Linear(in_features=size, out_features=4096),
            nn.Dropout(inplace=True),
            nn.Linear(in_features=4096, out_features=grid_num*grid_num*26)
        )
    ...
    def forward(self, x):
        ...
        x = self.yolo(x)
        x = torch.sigmoid(x) 
        x = x.view(-1,self.grid_num,self.grid_num,26)
        return x
